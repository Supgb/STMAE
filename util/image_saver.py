...
def get_image_array(images, grid_shape, captions={}):
    ...
    output_image = np.zeros([w*cate_counts, h*(rows_counts+1), 3], dtype=np.float32)
    col_cnt = 0
...
def pool_pairs(images, size, num_objects, max_frames=None, step=1, mode='train'):
    ...
    b = min(2, b)
    t = min(max_frames, t) if max_frames is not None else t
    ...
    for bi in range(b):
        for ti in range(0, t, step):
            ...
            for oi in range(max_num_objects):
                ...
                req_images['F0_Pseudo_mask_%d'%oi].append(mask_transform(images['f0_pseudo_masks_%d'%ti][bi][oi], size))
                if mode == 'train':
                    req_images['Query_mask_%d'%oi].append(mask_transform(images['query_masks_%d'%ti][bi][oi], size))
                    req_images['Query_pseudo_mask_%d'%oi].append(mask_transform(images['query_pseudo_masks_%d'%ti][bi][oi], size))
