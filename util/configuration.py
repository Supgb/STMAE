...
class Configuration():
    def parse(self, unknown_arg_ok=False):
        ...
        parser.add_argument('--video_len', help='Number of frames in a video', type=int, default=5)
        parser.add_argument('--dataset_root', help='Dataset root', default='../')
        ...
        parser.add_argument('--total_epoch', default=800, type=int)
        parser.add_argument('--val_mem_frames', default=5, type=int)
        ...
        parser.add_argument('--s3_batch_size', default=32, type=int)
        parser.add_argument('--s3_iterations', default=150000, type=int)
        # fine-tune means fewer augmentations to train the sensory memory
        parser.add_argument('--s3_finetune', default=20, type=int)
        parser.add_argument('--s3_steps', nargs="*", default=[85], type=int)
        parser.add_argument('--s3_lr', help='Initial learning rate', default=2e-5, type=float)
        parser.add_argument('--s3_num_ref_frames', default=3, type=int)
        parser.add_argument('--s3_num_frames', default=8, type=int)
        parser.add_argument('--s3_start_warm', default=4600, type=int)
        parser.add_argument('--s3_end_warm', default=18400, type=int)

        parser.add_argument('--gamma', help='LR := LR*gamma at every decay step', default=0.1, type=float)
        parser.add_argument('--weight_decay', default=0.05, type=float)

        # Loading
        parser.add_argument('--load_network', default=None, help='Path to pretrained network weight only')
        parser.add_argument('--load_checkpoint', default=None, help='Path to the checkpoint file, including network, optimizer and such')

        # Logging information
        parser.add_argument('--log_text_interval', default=150, type=int)
        parser.add_argument('--log_image_interval', default=500, type=int)
        parser.add_argument('--val_epochs', default=5, type=int)
        parser.add_argument('--save_network_interval', default=25, type=int)
        parser.add_argument('--save_checkpoint_interval', default=25, type=int)
